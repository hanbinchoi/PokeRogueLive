...
describe('<Move />', () => {
  ...
  const mockMoveUrl = 'https://pokeapi.co/api/v2/move/1/';

  const queryClient = new QueryClient();

  beforeEach(() => {
    // API 인터셉트 설정
    cy.intercept('GET', mockMoveUrl, {
      statusCode: 200,
      body: mockMoveData,
    }).as('getMoveData');
  });

  it('로딩 중에 로딩 컴포넌트가 표시된다', () => {
    cy.mount(
      <QueryClientProvider client={queryClient}>
        <Move levelLearnedAt={10} url={mockMoveUrl} />
      </QueryClientProvider>,
    );
    cy.get('.loading-component').should('exist');
  });

  it('API 호출 에러 발생 시 에러 메시지가 표시된다', () => {
    cy.mount(
      <QueryClientProvider client={queryClient}>
        <Move levelLearnedAt={10} url={'url/error'} />
      </QueryClientProvider>,
    );
    cy.wait(5000);
  ...
  it('모바일 화면에서 기술 정보가 올바르게 표시되는지 확인', () => {
    cy.viewport('iphone-6'); // 모바일 화면 크기로 변경
    ...
    // 모바일 화면에서 기술 정보가 표시되는지 확인
    cy.contains('위력').should('be.visible');
    cy.contains('명중률').should('be.visible');
    cy.contains('pp').should('be.visible');
  });

  it('데스크탑 화면에서 기술 정보가 올바르게 표시되는지 확인', () => {
    cy.viewport(1280, 800); // 데스크탑 화면 크기로 변경
    ...
    // 데스크탑 화면에서 기술 정보가 표시되는지 확인
    cy.contains('위력 - 40').should('be.visible');
    cy.contains('명중률 - 100').should('be.visible');
    cy.contains('pp - 35').should('be.visible');
  });
});
